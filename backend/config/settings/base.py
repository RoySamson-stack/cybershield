...
if SUPABASE_DB_URL:
    ...
    if match:
        DATABASES["default"]["HOST"] = match.group("host")
        DATABASES["default"]["PORT"] = match.group("port") or "5432"
        DATABASES["default"]["NAME"] = match.group("name")
        if match.group("user"):
            DATABASES["default"]["USER"] = match.group("user")
        if match.group("password"):
            DATABASES["default"]["PASSWORD"] = match.group("password")
else:
    DATABASES = {
        "default": {
            "ENGINE": "django.db.backends.postgresql",
...
SIMPLE_JWT = {
    "ACCESS_TOKEN_LIFETIME": timedelta(
        minutes=int(env("JWT_ACCESS_TOKEN_LIFETIME", default=15))
    ),
    "REFRESH_TOKEN_LIFETIME": timedelta(
        days=int(env("JWT_REFRESH_TOKEN_LIFETIME", default=7))
    ),
...
CORS_ALLOWED_ORIGINS = env.list(
    "CORS_ALLOWED_ORIGINS", default=["http://localhost:3001", "http://127.0.0.1:3001"]
)
